    ...
    <body>
        ...
        <section>
            <h1>
             <?php 
                if (isset($send) AND $send) {
                    echo '<img src="uploads/'.$newImageName.'"alt=SharlFiles" style="max-width : 75%">';
                }else{
            ...
            <?php 
                if (isset($send) AND $send) {
                    ?>
                        <h2>Fichier evoyé avec succès !</h2>
                        <p>Retrouvez ci-desous le liens vers votre fichier :</p>
                        <input type="text" name="" id="link" value="http://localhost/uploads <?= $newImageName ?>" readonly>
                    <?php  
                }else{
                    ?>
